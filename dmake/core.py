...
def release(options):
    ...
    release_list = {}
    for release in github.releases(owner, app):
    ...
    questions = [
        inquirer.List(
            ...
            choices=[release_list[key]['tag'] for key in sorted_release_keys if key[0:2] in latest_per_major_minor and latest_per_major_minor[key[0:2]] == key] + ['Other'],
        ),
    ]
    ...
    if answers['prev'] == 'Other':
        questions = [
            inquirer.List(
                'prev',
                message="Which was the previously released version?",
                choices=[release_list[key]['tag'] for key in sorted_release_keys],
                carousel=True
            ),
        ]
        answers = inquirer.prompt(questions)
    ...
    # Creates the release
    github.create_releases(owner, app, next_version, branch, change_log, prerelease)
    print("Done ! Check it at: https://github.com/{owner}/{repo}/releases/tag/{tag}".format(owner=owner, repo=app, tag=next_version))
