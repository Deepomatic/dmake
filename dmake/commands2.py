...
class CommandsManager(object):
    """ This class that handles command nodes and ouputs them to in a specific order.
        The script will be written to the output in this order:
            ...
            for each node in nodes:
                ...
                node.trailer()
            _write_trailer()
       Us this scheme to put the code you want to generate in the proper place.
    """
    ...
    def _write_header(self, context):
        ...
        # TODO: for the moment we only have one node
        node = self.nodes[0]
        node.generate_command(context, 'global_env', var="REPO", value=common.repo)
        node.generate_command(context, 'global_env', var="COMMIT", value=common.commit_id)
        node.generate_command(context, 'global_env', var="BUILD", value=common.build_id)
        node.generate_command(context, 'global_env', var="BRANCH", value=common.branch)
        node.generate_command(context, 'with_env', value=[("DMAKE_TMP_DIR", common.tmp_dir)])
        # check DMAKE_TMP_DIR still exists: detects unsupported jenkins reruns: clear error
        node.generate_command(context, 'sh', shell='dmake_check_tmp_dir')

    def _write_trailer(self, context):
        """
        The commands to be generated at the end of the file
        """
        node = self.nodes[0]
        node.generate_command(context, 'with_env_end')
