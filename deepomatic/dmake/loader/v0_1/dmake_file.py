...
class DMakeFile(DMakeFileAbstract):
    ...
    def register_services(self):
        for s in self.serializer.services:
            ...
            self._register_service_(s.service_name, s.needed_services, test_dependencies)

        for l in self.serializer.docker_links:
            self._register_service_(l.link_name)
    ...
    class Base(Action):
        ...
        def _generate_(self, dmake_file, _):
            commands, image_name = dmake_file.serializer.docker._serialize_(self.__path__)

    class BaseDocker(Action):
        stage = 'base'
        def _generate_(self, dmake_file, service):
            base = self.request('Base', service.get_name())
